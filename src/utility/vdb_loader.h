...
public:
	...
	struct Pool0Data
	{
		...
		glm::vec3 index;	// bounding box's min corner
	...
	// e.g. relative index (1, 2, 0) -> 1 * 2^4 * 2^4 + 2 * 2^4 + 0 + childoffset
	struct Pool1Data
	{
		uint32_t pool0Index; // points to the index in next level pool0
	};
	...
	struct Level1Data
	{
		...
		std::vector<Pool1Data> level1Pool1; // points to leaf nodes
