...
vec2 GetDistort(in vec2 uv)
{
    vec2 distort = texture(texDistortUV, inTexCoord).rg;
    return distort;
    //return vec2(0.0f);
}
...
void main(){
    ...
    vec2 uv = clamp(GetDistort(inTexCoord) + inTexCoord, vec2(0.0f, 0.0f), vec2(1.0f, 1.0f));
    float variance = texture(texDistortUV, inTexCoord).b;
    float mipLevel = variance;//log(variance) / log(2);
